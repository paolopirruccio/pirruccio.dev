...
export function AgencyBlurHero({lang}:{lang:Lang}){
  ...
  useEffect(()=>{const el=host.current,c=canvas.current,fb=fallback.current;if(!el||!c||!fb)return;const reduced=matchMedia("(prefers-reduced-motion: reduce)").matches;const mobile=matchMedia("(max-width: 700px), (pointer: coarse)").matches;const staticMode=reduced;
    if(mobile){
      ...
      const paint=(panelIndex:number,categoryIndex=0,animate=true)=>{const panel=panels[panelIndex],category=panel.categories?.[categoryIndex];setIndex(panelIndex);el.style.setProperty("--panel-bg",panel.bg);el.style.setProperty("--panel-fg",panel.fg);fb.style.color=panel.fg;fb.textContent=category?`${panel.line} ${category}`:panel.line;if(animate&&!reduced)fb.animate([{opacity:.15,transform:"translateY(calc(-2.5svh + 12px)) scale(.985)"},{opacity:1,transform:"translateY(-2.5svh) scale(1)"}],{duration:440,easing:"cubic-bezier(.22,1,.36,1)"})};
      const schedule=()=>{if(disposed)return;currentCategory=0;paint(currentPanel,0,currentPanel!==0);const panel=panels[currentPanel];if(panel.categories?.length&&!reduced)categoryTimer=window.setInterval(()=>{currentCategory=(currentCategory+1)%panel.categories!.length;paint(currentPanel,currentCategory)},520);if(!reduced)panelTimer=window.setTimeout(()=>{window.clearInterval(categoryTimer);currentPanel=(currentPanel+1)%panels.length;schedule()},panel.hold??4400)};
      schedule();
      return()=>{disposed=true;window.clearTimeout(panelTimer);window.clearInterval(categoryTimer);fb.getAnimations().forEach(animation=>animation.cancel())};
    }
    ...
    const categoryDelays=[1250,980,760,580,440,330,250,190,145,110,85,65,50,38,30];const loop=(now:number)=>{if(mobile&&now-lastPaint<50){raf=requestAnimationFrame(loop);return}lastPaint=now;const dt=Math.min(.05,Math.max(.001,(now-last)/1000));last=now;clock+=dt;const target=phase==="out"?0:1,k=phase==="out"?60:42,damp=2*Math.sqrt(k);velocity+=(-k*(progress-target)-damp*velocity)*dt;progress+=velocity*dt;if(phase==="in"&&progress>=.992){phase="hold";phaseStart=now;category=0;nextCategoryAt=now+categoryDelays[0]}else if(phase==="hold"){const categories=panels[current].categories;if(categories&&category<categories.length-1&&now>=nextCategoryAt){category++;mount(current);nextCategoryAt=now+categoryDelays[Math.min(category,categoryDelays.length-1)]}if(now-phaseStart>(panels[current].hold??4400)){phase="out";phaseStart=now}}else if(phase==="out"&&progress<=.02&&now-phaseStart>=40){current=(current+1)%panels.length;category=0;seed=Math.random()*100;mount(current);progress=0;velocity=0;phase="in";phaseStart=now}const p=Math.max(0,Math.min(1,progress));draw(p);if(!gl){fb.style.opacity=String(p);fb.style.filter=`blur(${(1-p)*10}px)`}if(onScreen&&!document.hidden)raf=requestAnimationFrame(loop);else raf=0};
  ...
  const p=panels[index];return <header className="studio-cycle-hero" ref={host} style={{"--panel-bg":p.bg,"--panel-fg":p.fg} as CSSProperties}>
    <canvas ref={canvas} aria-hidden/><div className="studio-cycle-fallback" ref={fallback}>{p.line}</div>
    <div className="studio-cycle-bottom"><p>{lang==="it"?"Siti su misura, senza sorprese. Sai prima cosa farò, quanto spenderai e quando sarà online.":"Tailored websites, no surprises. You know upfront what I’ll make, what you’ll spend and when it will be online."}</p></div>
  </header>;
}
