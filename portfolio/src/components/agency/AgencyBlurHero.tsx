...
export function AgencyBlurHero({lang}:{lang:Lang}){
  ...
  useEffect(()=>{const el=host.current,c=canvas.current,fb=fallback.current;if(!el||!c||!fb)return;const reduced=matchMedia("(prefers-reduced-motion: reduce)").matches;const mobile=matchMedia("(max-width: 700px), (pointer: coarse)").matches;const staticMode=reduced;
    if(mobile){
      ...
      const paint=(panelIndex:number,categoryIndex=0,animate=true)=>{const panel=panels[panelIndex],category=panel.categories?.[categoryIndex];setIndex(panelIndex);el.style.setProperty("--panel-bg",panel.bg);el.style.setProperty("--panel-fg",panel.fg);fb.style.color=panel.fg;if(fixedLine)fixedLine.textContent=panel.line;if(changingLine){changingLine.textContent=category??"";changingLine.hidden=!category}if(animate&&!reduced){const target=category&&categoryIndex>0?changingLine:fb;target?.animate([{opacity:.12,transform:"translateY(10px) scale(.985)"},{opacity:1,transform:"translateY(0) scale(1)"}],{duration:category&&categoryIndex>8?150:300,easing:"cubic-bezier(.22,1,.36,1)"})}};
